...
// #implementacion mmap: lseek es funcion del host que usan mmap y msync para posicionar el cursor del archivo
...
uint64_t  get_mmap_next(uint64_t* context);
void      set_mappings(uint64_t* context, uint64_t* mappings);
void      set_mmap_next(uint64_t* context, uint64_t next);

// #implementacion mmap: accesores del nodo de mapping (vaddr, length, prot, fd, offset) de la lista mappings de cada proceso
uint64_t* allocate_mapping();
uint64_t* get_next_mapping(uint64_t* mapping);
uint64_t  get_mapping_vaddr(uint64_t* mapping);
...
void      set_mapping_vaddr(uint64_t* mapping, uint64_t vaddr);
...
// #implementacion mmap: busca el mapping cuyo rango contiene vaddr, usado por is_valid_segment_read/write y copy_buffer
uint64_t* find_mapping_for_address(uint64_t* context, uint64_t vaddr);

// #implementacion mmap: valida que addr y length formen una region valida (alineada, sin overlap, dentro del espacio virtual) antes de crear el mapping
uint64_t is_valid_mmap_region(uint64_t* context, uint64_t addr, uint64_t length);

// #implementacion mmap: page cache global compartido entre procesos, registra que cache frame contiene cada pagina (fd, offset) de un archivo
uint64_t* allocate_cache_entry();
uint64_t* get_next_cache_entry(uint64_t* entry);
uint64_t  get_cache_entry_fd(uint64_t* entry);
uint64_t  get_cache_entry_offset(uint64_t* entry);
uint64_t  get_cache_entry_frame(uint64_t* entry);
void      set_next_cache_entry(uint64_t* entry, uint64_t* next);
void      set_cache_entry_fd(uint64_t* entry, uint64_t fd);
void      set_cache_entry_offset(uint64_t* entry, uint64_t offset);
...
// #implementacion mmap: cabeza del page cache global, compartida por todos los procesos para que compartan frames del mismo archivo/offset
...
// #implementacion mmap: contadores de lecturas/escrituras en la region mmap, mismo patron que data/stack/heap
...
void reset_segments_profile() {
  ...
  heap_reads   = 0;
  heap_writes  = 0;
  // #implementacion mmap: reinicia los contadores de acceso a la region mmap
  mmap_reads   = 0;
  mmap_writes  = 0;
}
...
// #implementacion mmap: contexto extendido a 40 entradas para guardar mappings y mmap_next de cada proceso
...
// #implementacion mmap: getters de mappings (entrada 38) y mmap_next (entrada 39)
...
// #implementacion mmap: setters de mappings (entrada 38) y mmap_next (entrada 39)
void set_mappings(uint64_t* context, uint64_t* mappings) { *(context + 38) = (uint64_t) mappings; }
void set_mmap_next(uint64_t* context, uint64_t next)      { *(context + 39) = next; }
...
uint64_t copy_buffer(uint64_t* context, uint64_t vbuffer, uint64_t* buffer, uint64_t size, uint64_t upload) {
  ...
  while (vaddr - vbuffer < size) {
    if (is_virtual_address_valid(vaddr, WORDSIZE)) {
      ...
      if (is_known_segment) {
        // #implementacion mmap: aplica el permiso (prot) del mapping tambien a los read/write del kernel emulado, no solo a los store/load de la CPU
...
void implement_fork(uint64_t* context) {
	...
	// #implementacion mmap: variables usadas para copiar context->mappings del padre al hijo en fork
	uint64_t* parent_mapping;
	uint64_t* child_mapping;
	uint64_t  j;
	uint64_t  mapped_page;
	uint64_t  number_of_mapped_pages;
	...
	// #implementacion mmap: el hijo hereda mmap_next y cada mapping del padre, compartiendo el mismo cache frame via set_PTE_for_page en vez de duplicarlo
	...
	while (parent_mapping != (uint64_t*) 0) {
		...
		while (j < number_of_mapped_pages) {
			...
			set_PTE_for_page(get_pt(child), mapped_page, get_frame_for_page(get_pt(context), mapped_page));
...
// #implementacion mmap: carga los 5 parametros (addr,length,prot,fd,offset) a A0..A4 y emite el ecall de mmap
void emit_mmap() {
...
// #implementacion mmap: crea el mapping, valida la region y mapea cada pagina virtual a su cache frame
void implement_mmap(uint64_t* context) {
  uint64_t  addr;
  uint64_t  length;
  uint64_t  prot;
  ...
  if (debug_syscalls) {
    ...
    print_register_value(REG_A2);
    printf(",");
    print_register_value(REG_A3);
    printf(",");
    print_register_value(REG_A4);
    printf(" |- ");
    print_register_value(REG_A0);
  }

  // paso 1: leer los 5 argumentos desde los registros donde el compilador los dejo
  ...
  // paso 2: redondear length hacia arriba al multiplo de PAGESIZE mas cercano
  length = round_up(length, PAGESIZE);

  // paso 3: si addr llego como 0, usar la siguiente direccion libre del proceso
  if (addr == 0)
    addr = get_mmap_next(context);
  ...
  if (is_valid_mmap_region(context, addr, length) == 0) {
    *(get_regs(context) + REG_A0) = sign_shrink(-1, SYSCALL_BITWIDTH);
    ...
    if (debug_mmap)
      printf("%s: mmap rejected invalid region at 0x%08lX of %lu bytes\n", selfie_name, addr, length);
  ...
  // paso 4: calcular cuantas paginas cubre el mapping
  number_of_pages = length / PAGESIZE;

  // paso 5: por cada pagina, obtener/crear su cache frame y mapear la pagina
  // virtual a ese frame usando la operacion de bajo nivel set_PTE_for_page()
  i = 0;

  while (i < number_of_pages) {
    page = get_page_of_virtual_address(addr) + i;

    // offset, multiplo de PAGESIZE, de esta pagina dentro del archivo
    ...
    // hi_page, asi que fork comparte el frame en vez de duplicarlo (M7)
    set_PTE_for_page(get_pt(context), page, frame);

    if (debug_mmap)
      printf("%s: mmap mapped page 0x%04lX (virtual address 0x%08lX) to frame 0x%08lX\n", selfie_name,
        page, get_virtual_address_of_page_start(page), frame);

    i = i + 1;
  }

  // paso 6: avanzar la direccion libre del proceso para que el siguiente
  // mmap con addr=0 no colisione con este mapping
  if (addr + length > get_mmap_next(context))
    set_mmap_next(context, addr + length);

  // paso 7: registrar el mapping al frente de context->mappings
  mapping = allocate_mapping();

  set_mapping_vaddr(mapping, addr);
  ...
  // paso 8: devolver la direccion inicial del mapping como resultado de la syscall
  *(get_regs(context) + REG_A0) = addr;

  set_pc(context, get_pc(context) + INSTRUCTIONSIZE);

  if (debug_mmap)
    printf("%s: mmap created mapping of %lu bytes at virtual address 0x%08lX for fd %lu at file offset %lu with prot %lu\n", selfie_name,
      length, addr, fd, offset, prot);
...
// #implementacion munmap: carga el unico parametro (addr) a A0 y emite el ecall de munmap; length se obtiene del mapping guardado
void emit_munmap() {
...
// #implementacion munmap: busca el mapping por addr, desmapea sus paginas de la tabla y elimina el nodo de context->mappings
void implement_munmap(uint64_t* context) {
  uint64_t  addr;
  uint64_t* mapping;
  uint64_t* previous_mapping;
  uint64_t* found_mapping;
  uint64_t  number_of_pages;
  uint64_t  page;
  uint64_t  i;
  ...
  // paso 1: leer la direccion recibida desde el registro correspondiente
  addr = *(get_regs(context) + REG_A0);

  // paso 2: recorrer context->mappings buscando addr exacta, manteniendo
  // una referencia al nodo anterior para poder desconectarlo despues
  previous_mapping = (uint64_t*) 0;
  found_mapping = (uint64_t*) 0;
  ...
  if (mapping != (uint64_t*) 0) {
    // paso 3: limpiar la tabla de paginas de cada pagina del mapping
    ...
    while (i < number_of_pages) {
      ...
      // frame = 0: esta es la operacion de bajo nivel que deja la pagina
      // como "no mapeada", sin tocar lowest/highest_lo_page ni el frame
      // fisico (que puede seguir compartido con otro proceso)
      set_PTE_for_page(get_pt(context), page, 0);

      i = i + 1;
    }

    // paso 4: desconectar el nodo de context->mappings
    if (previous_mapping == (uint64_t*) 0)
      set_mappings(context, get_next_mapping(mapping));
    else
      set_next_mapping(previous_mapping, get_next_mapping(mapping));

    // paso 5: exito
    *(get_regs(context) + REG_A0) = 0;

    if (debug_mmap)
      printf("%s: munmap removed mapping of %lu bytes at virtual address 0x%08lX\n", selfie_name,
        number_of_pages * PAGESIZE, addr);
  } else {
    // paso 5: error; sign_shrink(-1, SYSCALL_BITWIDTH) codifica -1 en
    // SYSCALL_BITWIDTH bits, igual convencion que usan open/read/write
    *(get_regs(context) + REG_A0) = sign_shrink(-1, SYSCALL_BITWIDTH);

    if (debug_mmap)
      printf("%s: munmap found no mapping at virtual address 0x%08lX\n", selfie_name,
        addr);
  }
...
// #implementacion msync: carga el unico parametro (addr) a A0 y emite el ecall de msync, mismo patron que munmap
void emit_msync() {
...
// #implementacion msync: busca el mapping por addr y escribe cada cache frame al archivo con lseek/write del host, unico punto que persiste cambios a disco
void implement_msync(uint64_t* context) {
  uint64_t  addr;
  uint64_t* mapping;
  uint64_t* found_mapping;
  ...
  // paso 1: leer la direccion recibida
  addr = *(get_regs(context) + REG_A0);

  // paso 2: buscar en context->mappings la entrada cuya direccion inicial
  // coincide con addr (no hace falta desconectarla, a diferencia de munmap)
  found_mapping = (uint64_t*) 0;
  mapping = get_mappings(context);
  ...
  if (mapping == (uint64_t*) 0) {
    // paso 3: error; sign_shrink(-1, SYSCALL_BITWIDTH) codifica -1 en el
    // ancho de la syscall, misma convencion que munmap/open
    *(get_regs(context) + REG_A0) = sign_shrink(-1, SYSCALL_BITWIDTH);

    if (debug_mmap)
      printf("%s: msync found no mapping at virtual address 0x%08lX\n", selfie_name,
        addr);
  } else {
    // paso 4: extraer fd, offset base y cantidad de paginas del mapping
    fd          = get_mapping_fd(mapping);
    base_offset = get_mapping_offset(mapping);

    number_of_pages = get_mapping_length(mapping) / PAGESIZE;

    // paso 5: por cada pagina, volcar el contenido de su frame al archivo
    i = 0;
    ...
    while (i < number_of_pages) {
      ...
      // asume IS64BITTARGET==1 (WORDSIZE==sizeof(uint64_t)), igual que M3
      write(fd, (uint64_t*) frame, PAGESIZE);

      if (debug_mmap)
        printf("%s: msync wrote frame 0x%08lX back to fd %lu at file offset %lu\n", selfie_name,
          frame, fd, file_offset);
...
void print_register_memory_profile() {
  ...
  // #implementacion mmap: imprime el perfil de lecturas/escrituras de la region mmap
  print_access_profile("mmap region:   ", "", mmap_reads, mmap_writes);
...
void init_context(uint64_t* context, uint64_t* parent, uint64_t* vctxt) {
  ...
  // #implementacion mmap: todo proceso nuevo inicia sin mappings y con mmap_next en MMAP_BASE; implement_fork sobreescribe esto en hijos
...
uint64_t* allocate_mapping() {
  return smalloc(MAPPINGENTRIES * sizeof(uint64_t));
}

uint64_t* get_next_mapping(uint64_t* mapping)   { return (uint64_t*) *mapping; }
uint64_t  get_mapping_vaddr(uint64_t* mapping)  { return             *(mapping + 1); }
uint64_t  get_mapping_length(uint64_t* mapping) { return             *(mapping + 2); }
uint64_t  get_mapping_prot(uint64_t* mapping)   { return             *(mapping + 3); }
uint64_t  get_mapping_fd(uint64_t* mapping)     { return             *(mapping + 4); }
uint64_t  get_mapping_offset(uint64_t* mapping) { return             *(mapping + 5); }

void set_next_mapping(uint64_t* mapping, uint64_t* next)    { *mapping       = (uint64_t) next; }
void set_mapping_vaddr(uint64_t* mapping, uint64_t vaddr)   { *(mapping + 1) = vaddr; }
void set_mapping_length(uint64_t* mapping, uint64_t length) { *(mapping + 2) = length; }
void set_mapping_prot(uint64_t* mapping, uint64_t prot)      { *(mapping + 3) = prot; }
void set_mapping_fd(uint64_t* mapping, uint64_t fd)          { *(mapping + 4) = fd; }
void set_mapping_offset(uint64_t* mapping, uint64_t offset)  { *(mapping + 5) = offset; }

// #implementacion mmap: busca el mapping cuyo rango [vaddr, vaddr+length) contiene la direccion recibida
uint64_t* find_mapping_for_address(uint64_t* context, uint64_t vaddr) {
  uint64_t* mapping;
...
// #implementacion mmap: valida que addr y length sean una region valida (alineada, dentro del espacio virtual, sin overlap) antes de mapear
uint64_t is_valid_mmap_region(uint64_t* context, uint64_t addr, uint64_t length) {
  uint64_t* mapping;

  if (length == 0)
    return 0; // mapping vacio, no tiene sentido crearlo

  if (addr % PAGESIZE != 0)
    return 0; // addr debe estar alineada a pagina (ver get_page_of_virtual_address)

  if (addr < MMAP_BASE)
    return 0; // no debe invadir code/data/heap (heap termina antes de MMAP_BASE)

  if (addr + length < addr)
    return 0; // overflow de addr + length

  if (addr + length - WORDSIZE > HIGHESTVIRTUALADDRESS)
    return 0; // fuera del espacio de direcciones virtuales

  if (addr + length > *(get_regs(context) + REG_SP))
    return 0; // no debe invadir el stack actualmente en uso (SP)

  mapping = get_mappings(context);

  while (mapping != (uint64_t*) 0) {
    // solapa si los rangos [addr,addr+length) y [vaddr,vaddr+len) se cruzan
    if (addr < get_mapping_vaddr(mapping) + get_mapping_length(mapping))
      if (addr + length > get_mapping_vaddr(mapping))
        return 0; // se solapa con un mapping ya existente
...
// -----------------------------------------------------------------
// #implementacion mmap: PAGE CACHE, estructura y accesores de cada entrada del page cache global
// -----------------------------------------------------------------
...
uint64_t* allocate_cache_entry() {
  return smalloc(CACHEENTRYENTRIES * sizeof(uint64_t));
}

uint64_t* get_next_cache_entry(uint64_t* entry)   { return (uint64_t*) *entry; }
uint64_t  get_cache_entry_fd(uint64_t* entry)     { return             *(entry + 1); }
uint64_t  get_cache_entry_offset(uint64_t* entry) { return             *(entry + 2); }
uint64_t  get_cache_entry_frame(uint64_t* entry)  { return             *(entry + 3); }

void set_next_cache_entry(uint64_t* entry, uint64_t* next)    { *entry       = (uint64_t) next; }
void set_cache_entry_fd(uint64_t* entry, uint64_t fd)         { *(entry + 1) = fd; }
void set_cache_entry_offset(uint64_t* entry, uint64_t offset) { *(entry + 2) = offset; }
void set_cache_entry_frame(uint64_t* entry, uint64_t frame)   { *(entry + 3) = frame; }

// #implementacion mmap: devuelve el frame cacheado para (fd, offset) si existe, o reserva uno con palloc() y carga la pagina del archivo si no
uint64_t get_or_load_cache_frame(uint64_t fd, uint64_t file_offset) {
  uint64_t* entry;
  uint64_t* frame;
  uint64_t  bytes_read;
  ...
  while (entry != (uint64_t*) 0) {
    if (get_cache_entry_fd(entry) == fd)
      if (get_cache_entry_offset(entry) == file_offset) {
        if (debug_mmap)
          printf("%s: page cache hit for fd %lu at file offset %lu, reusing frame 0x%08lX\n", selfie_name,
            fd, file_offset, get_cache_entry_frame(entry));

        return get_cache_entry_frame(entry);
      }

    entry = get_next_cache_entry(entry);
  }

  // cache miss: se reserva un frame fisico nuevo y se carga la pagina del archivo
  frame = palloc();

  lseek(fd, file_offset, SEEK_SET);

  // asume IS64BITTARGET==1 (WORDSIZE==sizeof(uint64_t)); en 32-bit
  // habria que decodificar palabra por palabra como hace copy_buffer
  bytes_read = read(fd, frame, PAGESIZE);

  if (debug_mmap)
    printf("%s: page cache miss for fd %lu at file offset %lu, read %lu bytes into frame 0x%08lX\n", selfie_name,
      fd, file_offset, bytes_read, (uint64_t) frame);
...
uint64_t is_stack_address(uint64_t* context, uint64_t vaddr) {
  ...
  if (vaddr >= *(get_regs(context) + REG_SP))
    if (vaddr <= HIGHESTVIRTUALADDRESS)
      // #implementacion mmap: excluye la zona de mmap del rango de stack, para que el crecimiento del stack no la pise
...
uint64_t is_address_between_stack_and_heap(uint64_t* context, uint64_t vaddr) {
  ...
  if (vaddr >= get_program_break(context))
    // #implementacion mmap: el heap no puede crecer dentro de la zona reservada para mmap (a partir de MMAP_BASE)
    if (vaddr < MMAP_BASE)
      if (vaddr < *(get_regs(context) + REG_SP))
        return 1;
...
uint64_t is_valid_segment_read(uint64_t vaddr) {
  ...
  if (is_data_address(current_context, vaddr)) {
    ...
    // #implementacion mmap: si la direccion no es data/stack/heap, revisa si pertenece a una region mmap con permiso de lectura
    mapping = find_mapping_for_address(current_context, vaddr);

    if (mapping != (uint64_t*) 0) {
      // permite lectura si prot no es exclusivo de escritura
...
uint64_t is_valid_segment_write(uint64_t vaddr) {
  ...
  if (is_data_address(current_context, vaddr)) {
    ...
    // #implementacion mmap: si la direccion no es data/stack/heap, revisa si pertenece a una region mmap con permiso de escritura
    mapping = find_mapping_for_address(current_context, vaddr);

    if (mapping != (uint64_t*) 0) {
      // la escritura solo es valida si el mapping se creo para escritura o
      // para lectura/escritura, no si se creo exclusivamente para lectura
      if (get_mapping_prot(mapping) != MMAP_PROT_READ) {
